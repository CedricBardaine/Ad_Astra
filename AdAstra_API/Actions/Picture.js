...
module.exports = {
    ...
    */
    insert_Picture: (req, res) => {
        const insertPict = "insertPicture: "
        ...
        form.on('error', function (err) {
            console.log(insertPict+ "error : "+err);
            res.writeHead(200, {'content-type': 'text/plain'});
            res.end('error:\n\n'+util.inspect(err));
        })
        .on('aborted', function (err) { console.log(insertPict+ "user aborted upload : "+err); })
        .on('end', function() { console.log(insertPict+ 'End of operation.'); })
        .on('field', (fieldName, fieldValue) => {
            if (fieldName != "id_userStar") console.log(insertPict+ "Error ! Wrong form field sent ! n°"+ (errorFieldCount++) );
            else {
                console.log(insertPict+ fieldName + " : "+ fieldValue);
                ...
                db.query(query, (err, result) => {
                    ...
                    form.on('file', function (field, file) {
                        //   console.log(insertPict+ "field : "+field, file);
                        console.log(insertPict+ "saving file...");
                    ...
                    form.parse(req, (err, fields, files) => {
                        ...
                        // console.log(insertPict+ JSON.stringify({ fields, files }, null, 2));
                        console.log(insertPict+ "saved picture to : "+imagePathAndName);
                    });
